...
# Tallentaa muutoksia tietokantaan
    sql = (f"UPDATE game "
...
# Antaa pelaajalle 100€ lisää rahaa. Voisi myös tulostaa jonkin selityksen mistä raha tulee.
    ...
    #Rahaa saa 3*gdp. Arvon muuttaminen vaikuttaa vaikeusasteeseen.
    gdp = sql_gdp[0][0]
    gdp_bonus = int(3 * gdp)
...
#Tarkistaa onko pelaaja käynyt kohteessa. Muokkaa tietokantaan käydyt kentät.
    ...
    for tuple in goal_list:
        ...
        if game_location in tuple and tuple[2]== "0":
            ...
            sql2 = f"UPDATE goal_reached SET reached = '1' WHERE game_id = '{game_id}' and ident = '{game_location}'  "
            kursori.execute(sql2)
            yhteys.commit()
...
def end_game(game_id):
    ...
    game_id, game_name, game_location, game_money, game_co2, game_money_gained, game_money_spent, game_distance, game_flights = game_values(
        game_id)

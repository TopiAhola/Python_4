...
class HashTable():
    ...
    def get(self, key, alternative=None):
        ...
        primary_item : HashItem | None = self.slots[index]
        primary_key = primary_item.key if primary_item else None
        primary_value = primary_item.value if primary_item else None

        if primary_key == key:
            return primary_value

        elif primary_key != key:
            #look all places
            next_index : int = self._find_key(index, key)
            if next_index and self.slots[next_index]:
                return self.slots[next_index].value

...
class HashTable():
    ...
    def put(self, key, value):
        ...
        #try updating old key
        existing_index : int = self._find_key( 0, key )
        if existing_index is not None:
            #print("found old slot with key")
            self.slots[existing_index] = HashItem(key, value)
            return

        #see if free slots
        if self.used_slots >= len(self.slots):
            raise MemoryError("Table is full")

        #get first free slot if new key
        index : int = self._hash(key)
        free_slot = self.slots[index]
        if free_slot is None:
            self.slots[index] = HashItem(key, value)
            self.used_slots += 1
            #print("found new slot with hash(key)")
            return

        #get any free slot
        free_index : int = self._find_free_slot(self._hash(key)) #
        if free_index is not None:
            #print("found new slot with find_free_slot()")
            self.slots[free_index] = HashItem(key, value)
            self.used_slots += 1
            return

        elif free_index == None:
            raise MemoryError("No free slot found")

        #program shouldn't run here ever
        raise RuntimeError("")
